...
pub fn angle_between<T>(a: Vec2<T>, b: Vec2<T>) -> T
	where T: Copy + Trig + Sqrt<T> + Mul<Output=T> + Div<Output=T> + Add<Output=T> + Sub<Output=T> + Abs {
	let (a, b) = (a.normalize(), b.normalize());
	dot(a, b).acos()
}

pub use crate::prelude::{dot,distance,distance_squared,orthog_dist};
impl<T: Copy + Mul<Output=T> + Add<Output=T> + Sub<Output=T> + Abs> Dot<T> for Vec2<T> {
